...
object LogAnalysisDB {
  ...
  /**
    ...
    * @return
    */
  def getPendingFilePath(spark:SparkSession,serverList:String) : List[String] = {
    ...
    for(i <- 0 until serverAry.length){
      //println(serverAry(i))
      for( days <- Range(1,4) ){
        val fileName:String = "/data/logHDFS/"+serverAry(i) +"/" + date.minusDays(days).toString("yyyy-MM-dd") + ".log"
        //println(fileName)
  ...
  def main(args: Array[String]): Unit = {
    ...
    //创建对象
    val files:List[String] = getPendingFilePath(spark,serverList)
...
//        }
    ...
    //归档
    if(iConfig.isArchive && isSuccess)
    {
      //得到归档名称
