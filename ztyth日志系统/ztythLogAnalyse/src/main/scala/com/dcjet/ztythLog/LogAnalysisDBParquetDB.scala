...
/**
  * 提交脚本
  * spark2-submit --class com.dcjet.ztythLog.LogAnalysisDBParquetDB --master yarn --deploy-mode cluster --jars /home/sparkjars/fastjson-1.2.47.jar /data/workspace/spark/ztythloganalyse.jar debug /data/logHDFS/apollo.archive/20181024081926.parquet/
  */
object LogAnalysisDBParquetDB {
  ...
  def saveByMap(df:DataFrame): Unit = {
    ...
    jobConf.set(org.apache.hadoop.hbase.mapred.TableOutputFormat.OUTPUT_TABLE, "T_BD_ZTYTH_LOG_DEBUG")
  ...
  def saveByPartition(df:DataFrame): Unit = {
    df.rdd.foreachPartition {
      y => {
        ...
        val myTable = new org.apache.hadoop.hbase.client.HTable(myConf, "T_BD_ZTYTH_LOG_DEBUG")
  ...
  def saveByBlukLoad(df:DataFrame): Unit = {
    ...
    val conf = HBaseConfiguration.create()
    val tableName = "T_BD_ZTYTH_LOG_DEBUG"
