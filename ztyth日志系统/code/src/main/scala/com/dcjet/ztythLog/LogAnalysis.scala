...
/**
  * 提交脚本
  * spark2-submit --class com.dcjet.ztythLog.LogAnalysis --master yarn --deploy-mode cluster --jars /home/sparkjars/fastjson-1.2.47.jar /data/workspace/spark/code.jar debug /data/logHDFS/apollo/2018-08-08.log
  */
object LogAnalysis {
  ...
  /**
    ...
    * @return
    */
  def getPendingFilePath(spark:SparkSession) : List[String] = {
    ...
    for( days <- Range(1,4) ){
      val fileName:String = "/data/logHDFS/apollo/" + date.minusDays(days).toString("yyyy-MM-dd") + ".log"
      //println(fileName)
  ...
  def en2cnForDelete(tableName:String,deleteRow:String,cacheEn2CnMap:Map[String,String]):String = {
    ...
    while (it.hasNext){
      ...
      if(cacheEn2CnMap.contains(key)) {
        ...
        resultString += cnString + "(" + item.toString() + ")" + ":" + delObj.get(item).toString() + ","
      }
      else
        {
          resultString += item.toString() +":" + delObj.get(item).toString() + ","
  ...
  def en2cnForUpdate(tableName:String,updateColumn:String,cacheEn2CnMap:Map[String,String]):String = {
    ...
    for(i<- 0 until length){
      ...
      var oldValue = item.get("OldValue").toString()
  ...
  def main(args: Array[String]): Unit = {
    ...
    //统一配置
    var iConfig:InnerConfig = InnerConfig(true,true,false,true)
    var debugLogPath = "/data/logHDFS/apollo/test.log"
    if(args.length>0 && args(0) == "debug") {
      iConfig = InnerConfig(false,true,true,true)
      if(args.length>1){
        debugLogPath=args(1)
      }
    }
    ...
    import org.joda.time._
    //创建对象
    val files:List[String] = getPendingFilePath(spark)
    var record: Record= null
    var logPath : String = null


    if(iConfig.isDebug ==false) {
      if (files.length == 0) {
        log("没有可分析的原日志文件，退出执行")
        val record1 = Record(DateTime.now().toString("yyyyMMddHHmmss"),
          DateTime.now().toString("yyyy-MM-dd HH:mm:ss"), DateTime.now().toString("yyyy-MM-dd HH:mm:ss"), 0, 0, 0, 0, "没有可分析的原日志文件，退出执行", "", "")
        //入库
        val df = Seq(record1).toDF()
        df.write.mode("append").saveAsTable("t_bd_record")
        return
      }
      record = Record(DateTime.now().toString("yyyyMMddHHmmss"),
        DateTime.now().toString("yyyy-MM-dd HH:mm:ss"),null,0,0,0,0,"", files.reduce(_+"," + _),"")
      logPath = record.originFile
    }
    else
      {
        record = Record(DateTime.now().toString("yyyyMMddHHmmss"),
          DateTime.now().toString("yyyy-MM-dd HH:mm:ss"), DateTime.now().toString("yyyy-MM-dd HH:mm:ss"), 0, 0, 0, 0, "test使用", "", "")
        logPath = debugLogPath
      }

    //val spark =org.apache.spark.sql.SparkSession.builder().enableHiveSupport().master("local[*]").getOrCreate()
    //import org.apache.spark.sql.SparkSession
    val originLines:org.apache.spark.rdd.RDD[String] = spark.sparkContext.textFile(logPath)
    ...
    val originalSource = originLines.map(line => {
      ...
      val pageCode = jsonContent.get("PageCode").toString()
      val pageName = jsonContent.get("PageName").toString()
      val functionCode = jsonContent.get("FunctionCode").toString()
      val functionName = jsonContent.get("FunctionName").toString()
      val doPerson = jsonContent.get("DoPerson").toString().split('|')(0)
      val ip = jsonContent.get("DoPersonIP").toString()
      val corpCode = jsonContent.get("CorpCode").toString()

      message = jsonContent.get("Message").toString()
    ...
    //归档
    if(iConfig.isArchive)
      {
        //得到归档名称
        val archiveName = "/data/logHDFS/apollo.archive/" + org.joda.time.DateTime.now().toString("yyyyMMddHHmmss") + ".parquet"
        originLines.toDF().write.option("compression","gzip").parquet(archiveName)

         //spark.read.parquet(archiveName)
        record.archivePath= archiveName
        if(iConfig.isDeleteOriginFile)
          {
            val hdfs = org.apache.hadoop.fs.FileSystem.get(spark.sparkContext.hadoopConfiguration)
            for(f:String <-files)
              {
